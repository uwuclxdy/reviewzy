/** The git hosts the dashboard recognizes, each with its own brand icon. */
...
/** A recognized git remote, reduced to what the repo row renders: the provider, the bare path, and the web link. */
export type RepoLink = {
  readonly provider: RepoProvider;
  /** The bare repo path shown in place of the remote, e.g. `uwuclxdy/clauth` — no scheme, host, or `.git`. */
  readonly path: string;
  /** The web URL the row links out to, always http(s). */
  readonly href: string;
};

/** The parsed pieces of a git remote, before provider detection decides whether to show it. */
type Remote = {
...
/**
 * Splits a git remote into its host and path, covering the two shapes git writes: the scp-like
 * `[user@]host:path` and the scheme-ful `https://`/`ssh://`/`git://` forms. Anything else — a local
 * path, a bare word — has no host to name, so it returns null.
 */
function splitRemote(raw: string): Remote | null {
  const value = raw.trim();
  if (value === "") return null;

  // scp-like: git@github.com:owner/repo.git — no scheme, and the colon sits before any slash, so
  // it is not the `://` of a scheme-ful remote.
  const scp = /^(?:[^@/]+@)?([^/:]+):(?!\/\/)(.+)$/.exec(value);
  if (scp !== null) {
    const host = scp[1];
    const path = scp[2];
    if (host === undefined || path === undefined) return null;
    return { host: host.toLowerCase(), path, scheme: "", port: "" };
  }
  ...
  return { host: url.hostname, path: url.pathname, scheme: url.protocol.replace(/:$/, ""), port: url.port };
}

/** Strips the URL's leading slash and git's trailing `.git`, leaving the bare owner/repo path. */
...
/**
 * Names the host's provider from its hostname. A host naming no known provider but still carrying
 * "git" (`git.example.com`, `git.kernel.org`) is a self-hosted git frontend, shown under the gitea
 * brand — the check runs last so the known hosts that also contain "git" match first.
 */
function detectProvider(host: string): RepoProvider | null {
  const h = host.toLowerCase();
  if (h.includes("github")) return "github";
  if (h.includes("gitlab")) return "gitlab";
  if (h.includes("codeberg")) return "codeberg";
  if (h.includes("forgejo")) return "forgejo";
  if (h.includes("gitea")) return "gitea";
  if (h.includes("bitbucket")) return "bitbucket";
  if (h.includes("sr.ht") || h.includes("sourcehut")) return "sourcehut";
  if (h.includes("gitee")) return "gitee";
  if (h.includes("dev.azure.com") || h.includes("visualstudio.com")) return "azure";
  if (h.includes("git")) return "gitea";
  return null;
}

/** The web URL to open, preferring the remote's own http(s) scheme and keeping a non-default port. */
function buildHref(remote: Remote, path: string): string {
  const scheme = remote.scheme === "http" || remote.scheme === "https" ? remote.scheme : "https";
  const authority = remote.port === "" ? remote.host : `${remote.host}:${remote.port}`;
  return `${scheme}://${authority}/${path}`;
}

/**
 * Recognizes a git remote into `{ provider, path, href }`, or null when the host is not a known
 * provider. Null means the repo row falls back to showing the raw string as filed.
 */
export function parseRepoLink(raw: string): RepoLink | null {
  const remote = splitRemote(raw);
  if (remote === null) return null;
  const path = cleanPath(remote.path);
  if (path === null) return null;
  const provider = detectProvider(remote.host);
  if (provider === null) return null;
  return { provider, path, href: buildHref(remote, path) };
}

/** The simple-icons brand glyph per provider, one `<path d>` each, drawn filled at 24×24. */
export const REPO_ICON: Record<RepoProvider, string> = {
  github:
