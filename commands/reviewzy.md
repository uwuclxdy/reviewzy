...
3. call `file_entries` with the slug and entries. each entry carries the exact `anchor_text`, a proposed `agent_draft`, and any `constraints`. one entry is one edit: `anchor_text` and `agent_draft` may span lines, so file a multi-line passage as a single entry.
